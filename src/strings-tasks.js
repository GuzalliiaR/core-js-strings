...
function containsSubstring(str, substring) {
  const res = str.includes(substring);
  return res;
}
...
function countVowels(str) {
  ...
  vowels.forEach((letter) => {
    if (str.includes(letter)) {
      arrayStr.forEach((letterInStr) => {
        if (letterInStr === letter) {
          count += 1;
        }
      });
    }
...
function isPalindrome(str) {
  ...
  const arrayFromStr = str
    .toLowerCase()
    .split('')
    .filter((element) => !garbage.includes(element));

  const newStr = arrayFromStr.join('');
  const reversNewStr = arrayFromStr.reverse().join('');

  return newStr === reversNewStr;
}
...
function findLongestWord(sentence) {
  ...
  const wordMaxLength = arrWords.find((word) => word.length === maxLength);
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    let codROT13 = unicode + 13;
    let dif;

    if (unicode >= 65 && unicode <= 90 && codROT13 > 90) {
      dif = codROT13 - 90;
      codROT13 = 64 + dif;
    } else if (unicode >= 97 && unicode <= 122 && codROT13 > 122) {
      dif = codROT13 - 122;
      codROT13 = 96 + dif;
    } else if (
      unicode < 65 ||
    ...
    codingStr = `${codingStr}${String.fromCodePoint(codROT13)}`;
  }
